...
class Kirki_Controls_Typography_Control extends Kirki_Customize_Control {
	...
	public function to_json() {
		...
		$this->json['value'] = array(
			'uppercase'      => isset( $value['uppercase'] ) ? $value['uppercase'] : '',
			'italic'         => isset( $value['italic'] ) ? $value['italic'] : '',
			'underline'      => isset( $value['underline'] ) ? $value['underline'] : '',
			'strikethrough'  => isset( $value['strikethrough'] ) ? $value['strikethrough'] : '',
		...
		<div class="wrapper">
			<# if ( data.choices['font-style'] ) { #>
				<div class="font-style">
					<div class="uppercase">
						<label for="uppercase_{{ data.id }}">
							<# if ( data.value['uppercase'] == '' && typeof data.default['font-style'] != 'undefined' && (data.default['font-style'].indexOf('uppercase') != -1) ) { data.value['uppercase'] = '1'; } #>
							<input name="uppercase_{{ data.id }}" id="uppercase_{{ data.id }}" type="checkbox" value="{{ data.value['uppercase'] }}" {{{ data.link }}}<# if ( '1' == data.value['uppercase'] ) { #> checked<# } #>>
							<span class="dashicons">
								<span class="dashicons-editor-textcolor"></span><span class="dashicons-editor-textcolor"></span>
							</span>
						</label>
					</div>
					<div class="italic">
						<label for="italic_{{ data.id }}">
							<# if ( data.value['italic'] == '' && typeof data.default['font-style'] != 'undefined' && (data.default['font-style'].indexOf('italic') != -1) ) { data.value['italic'] = '1'; } #>
					...
					<div class="underline">
						<label for="underline_{{ data.id }}">
							<# if ( data.value['underline'] == '' && typeof data.default['font-style'] != 'undefined' && (data.default['font-style'].indexOf('underline') != -1) ) { data.value['underline'] = '1'; } #>
					...
					<div class="strikethrough">
						<label for="strikethrough_{{ data.id }}">
							<# if ( data.value['strikethrough'] == '' && typeof data.default['font-style'] != 'undefined' && (data.default['font-style'].indexOf('strikethrough') != -1) ) { data.value['strikethrough'] = '1'; } #>
			...
			<# if ( data.choices['font-size'] ) { #>
				<div class="font-size">
					...
					<select>
						...
						<option value="vw" <# if ( units == 'vw' ) { #> selected <# } #>>vw</option>
						<option value="vh" <# if ( units == 'vh' ) { #> selected <# } #>>vh</option>
					</select>
				</div>
			<# } #>
